...
import {IWFTM} from "src/interfaces/IWFTM.sol";
import {IBaseV1Factory} from "src/interfaces/IBaseV1Factory.sol";
import {IBaseV1Pair} from "src/interfaces/IBaseV1Pair.sol";
...
contract BaseV1Router01 {
    ...
    receive() external payable {
        assert(msg.sender == address(i_wftm)); // only accept FTM via fallback from the WFTM contract
    }
    ...
    function removeLiquidityFTM(
        ...
        (amountToken, amountFTM) = removeLiquidity(
            token, address(i_wftm), stable, liquidity, amountTokenMin, amountFTMMin, address(this), deadline
        );
        _safeTransfer(token, to, amountToken);
        i_wftm.withdraw(amountFTM);
    ...
    /// @notice calculates the CREATE2 address for a pair without making any external calls1
        ...
        pair = address(
            uint160(
                uint256(
                    keccak256(
                        abi.encodePacked(
                            hex"ff", i_factory, keccak256(abi.encodePacked(token0, token1, stable)), pairCodeHash
                        )
                    )
                )
            )
    ...
    function getAmountsOut(uint256 amountIn, Route[] memory routes) public view returns (uint256[] memory amounts) {
        ...
        for (uint256 i = 0; i < routes.length; i++) {
            ...
            if (IBaseV1Factory(i_factory).isPair(pair)) {
    ...
    function addLiquidityFTM(
        ...
        address to,
        uint256 deadline
    ) external payable ensure(deadline) returns (uint256 amountToken, uint256 amountFTM, uint256 liquidity) {
        (amountToken, amountFTM) =
            _addLiquidity(token, address(i_wftm), stable, amountTokenDesired, msg.value, amountTokenMin, amountFTMMin);
        address pair = pairFor(token, address(i_wftm), stable);
        _safeTransferFrom(token, msg.sender, pair, amountToken);
        i_wftm.deposit{value: amountFTM}();
        assert(i_wftm.transfer(pair, amountFTM));
        liquidity = IBaseV1Pair(pair).mint(to);
    ...
    function removeLiquidityFTMWithPermit(
        ...
        IBaseV1Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
        (amountToken, amountFTM) =
            removeLiquidityFTM(token, stable, liquidity, amountTokenMin, amountFTMMin, to, deadline);
    }
    ...
    function swapExactFTMForTokens(uint256 amountOutMin, Route[] calldata routes, address to, uint256 deadline)
        external
        payable
        ensure(deadline)
        returns (uint256[] memory amounts)
    {
        require(routes[0].from == address(i_wftm), "BaseV1Router: INVALID_PATH");
        amounts = getAmountsOut(msg.value, routes);
        require(amounts[amounts.length - 1] >= amountOutMin, "BaseV1Router: INSUFFICIENT_OUTPUT_AMOUNT");
        i_wftm.deposit{value: amounts[0]}();
        assert(i_wftm.transfer(pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]));
        _swap(amounts, routes, to);
    }
    ...
    function swapExactTokensForFTM(
        ...
        require(routes[routes.length - 1].to == address(i_wftm), "BaseV1Router: INVALID_PATH");
        amounts = getAmountsOut(amountIn, routes);
        require(amounts[amounts.length - 1] >= amountOutMin, "BaseV1Router: INSUFFICIENT_OUTPUT_AMOUNT");
        _safeTransferFrom(
            routes[0].from, msg.sender, pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]
        ...
        i_wftm.withdraw(amounts[amounts.length - 1]);
    ...
    function isPair(address pair) external view returns (bool) {
        return IBaseV1Factory(i_factory).isPair(pair);
    }
    ...
    function quoteAddLiquidity(
        ...
        address _pair = IBaseV1Factory(i_factory).getPair(tokenA, tokenB, stable);
    ...
    /// @notice Given some amount of an asset and pair reserves, returns an equivalent amoutn of the other asset
    ...
    function _addLiquidity(
        ...
        require(amountADesired >= amountAMin, "BaseV1Router: INSUFFICIENT_A_AMOUNT");
        require(amountBDesired >= amountBMin, "BaseV1Router: INSUFFICIENT_B_AMOUNT");
        // create the pair if it doesn't exist yet
        address _pair = IBaseV1Factory(i_factory).getPair(tokenA, tokenB, stable);
        if (_pair == address(0)) {
            _pair = IBaseV1Factory(i_factory).createPair(tokenA, tokenB, stable);
        }
        (uint256 reserveA, uint256 reserveB) = getReserves(tokenA, tokenB, stable);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
    ...
    function _safeTransfer(address token, address to, uint256 value) internal {
        require(token.code.length > 0, "BaseV1Router: INVALID_TOKEN");
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(IERC20.transfer.selector, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), "BaseV1Router: TRANSFER_FAILED");
    }
    ...
    function _safeTransferFTM(address to, uint256 value) internal {
        ...
        require(success, "BaseV1Router: FTM_TRANSFER_FAILED");
    }
}
